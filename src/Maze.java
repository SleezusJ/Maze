    ...
    public static void main(String[] args)
    {
        ...
        if(robotActive)
        {
            new Thread(new Runnable(){
                public void run() {
                    ArrayList direcs = new ArrayList(Arrays.asList(RIGHT,RIGHT,DOWN));
                    //doMazeRandomWalk();
                    doMazeGuided(direcs);
                }
            }).start();
...
//		maze[0][0]^=LEFT;    //commented out for now so that robot doesn't run out the entrance
    ...
    public static ArrayList DFS(int x, int y){
        int iterations = 0;
    ...
    public static void doMazeGuided(ArrayList<Integer> directionList) {
        int dir = RIGHT;
        for(int i=0; i < directionList.size(); i++){
            ...
            // direction is that of directionList(i)
            ...
            mazecomp.repaint();
            try {
                Thread.sleep(SPEED); } catch (Exception e) { }
